...
3. Generation verifies the schema, reruns or verifies math validation, checks the contract hash, and only then converts the contract into browser HTML.
...
- Do not let `scripts/generate_simulation.py` accept stale or failed math output. The `contract_hash` in the math result must match the exact simulation JSON.
- Derive mandatory checks from simulation type and scene actions; fixtures cannot opt out of required validation by omitting checks.
- Scenes that affect concrete objects must use `targets` or `args`, never renderer guesses.
